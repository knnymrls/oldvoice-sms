  ...
  async handleIncomingSMS(phoneNumber, message) {
    try {
      ...
      if (normalizedMessage === 'reset' || normalizedMessage === '/reset') {
        ...
        // Also reset rate limit for testing
        const client = await redis.getClient();
        await client.del(`rate:${phoneNumber}`);
        logger.info('Rate limit reset for user', { phoneNumber });
  ...
  async completeConversation(user, phoneNumber, conversation) {
    try {
      logger.info('Completing conversation', { 
        ...
        storytellerPhone: conversation.data.storyteller.phone,
        relationship: conversation.data.storyteller.relationship,
        questionCount: conversation.data.questions.length,
        fullData: JSON.stringify(conversation.data)
      });
